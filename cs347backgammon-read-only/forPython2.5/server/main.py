...
def runGameServer(telnet_disabled, address, port):
    server = GameServer("slave", "slave-password")
    server.run(telnet_disabled, address, port)

@runAsMain
def runServerAndRedirect(telnet_disabled, address, port):
    Thread = threading.Thread
    redir = Thread(target=runRedirect, args=(telnet_disabled,address, port))
    redir.start()
    server = Thread(target=runGameServer, args=(telnet_disabled,address, port))
    server.start()
    redir.join()
    server.join()
